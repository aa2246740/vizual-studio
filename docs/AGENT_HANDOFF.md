...
- Agent 静默覆盖用户历史产物。
- 只生成看起来漂亮但不可审计的页面。
- 用自然语言猜 Design.md 结果；内部可以看映射诊断，但不要把分数或 mapping report 展示给普通用户。
...
2. PPT 编辑器：左侧页面缩略图、中央 16:9 画布、右侧 AI 协作流 + 当前对象微调。
3. 设计风格页：普通用户语言的风格控制，不直接暴露 `.md` 术语，也不向用户展示工程映射分数。
4. Edit 模式：文字块可直接在画布内编辑，右侧显示当前对象属性。
5. Draw / Click 模式：用户圈选区域或点选对象后，在画布内 popup 直接提交修改意见给 Agent。
